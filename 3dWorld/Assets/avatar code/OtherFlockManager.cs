...
public class OtherFlockManager : MonoBehaviour
{
	...
	public void Start ()
	{
		...
		OtherFlocker flocker; // reference to flocker scripts
		...
		for(int i = 0; i < numberOfFlockers; i++)
		{
			...
			flockers.Add((GameObject)Instantiate(flockerPrefab, 
				new Vector3(380,285,300 + 5*i), Quaternion.identity));
			//grab a component reference
			flocker = flockers[i].GetComponent<OtherFlocker>();
